    ...
    public class PayController : OwnBaseApiController
    {
        ...
        public APIResponse ReceiveNotifyByStar()
        {
            ...
            if (!BizFactory.AppSettings.IsTest)
            {
                ...
                string signStr = Signature.Compute("test", "6ZB97cdVz211O08EKZ6yriAYrHXFBowC", timespan, postData);
                Dictionary<string, string> headers = new Dictionary<string, string>();
                headers.Add("key", "test");
                headers.Add("timestamp", timespan.ToString());
                headers.Add("sign", signStr);
                HttpUtil http = new HttpUtil();
                http.HttpPostJson("http://120.79.233.231/api/Pay/ReceiveNotifyByStar", postData, headers);
